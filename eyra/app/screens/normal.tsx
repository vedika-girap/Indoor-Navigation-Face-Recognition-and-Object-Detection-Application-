...
export default function NormalModeScreen() {
  ...
  const captureAndDetect = async () => {
    if (cameraRef.current && !isProcessing) {
      ...
      try {
        ...
        const response = await fetch('http://YOUR_SERVER_IP:8000/object_detection/', {
          method: 'POST',
          headers: {
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.captureButton} onPress={captureAndDetect} disabled={isProcessing}>
        <Text style={{ color: 'white' }}>{isProcessing ? 'Processing...' : 'Detect Objects'}</Text>
      </TouchableOpacity>
      {detections.length > 0 && (
